...
from datasets.data_transforms import train_transform, val_transform, train_transform_vit, val_transform_vit
...
def make_dataloader(image_id, labels, train_idx, val_idx, cfg, mode):
    # 画像とラベルのデータセットを作成
    dataset = ImageDataset(image_id, labels, image_dir=cfg.dataset.image_dir, transform=None)

    # データセットの分割
    if mode=="Vit":
        train_dataset = CustomSubset(dataset, train_idx, transform=train_transform_vit)
        val_dataset = CustomSubset(dataset, val_idx, transform=val_transform_vit)
    else:
