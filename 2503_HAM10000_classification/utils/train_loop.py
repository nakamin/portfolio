...
def objective(cfg, train_loader, val_loader, num_classes, num_epochs, model_filename, mode):
    """
    訓練と推論をまわす
    推論の中でのベストモデルをロードして保存してからbest_model_stateを記録する
    """
    device = "cuda" if torch.cuda.is_available() else "cpu"

    model = create_model(cfg, mode).to(device)
