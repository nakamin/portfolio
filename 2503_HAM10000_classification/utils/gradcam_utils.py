...
def get_valdata(val_idx, dataset, image_id, device, model):
    ...
    correct_samples = dict() # {class_id: (image_path, idx, true_label, pred_label)}
    ...
    for idx in val_idx:
        ...
        if pred_label == true_label or true_label == 3:
            # キーはクラスID（true_label）、値は (画像, インデックス, 正解ラベル, 予測ラベル)
...
def get_heatmap(cfg, correct_samples, val_idx, val_subset, device, model):
    ...
    for class_id, (pil_image, idx, true_label, pred_label) in correct_samples.items():   
...
def visualize_heatmap(cfg, correct_samples, heatmaps, result):
    """
    heatmapを可視化する
    """
        
    for class_id, (pil_img, idx, true_label, pred_label) in correct_samples.items():  
        ...
        plt.savefig(os.path.join(output_dir, f"gradcam_cnn_{result}_idx_{idx}_true_{true_label}_pred_{pred_label}.png"))
